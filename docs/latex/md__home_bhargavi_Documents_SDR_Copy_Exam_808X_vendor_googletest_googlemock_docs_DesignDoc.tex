This page discusses the design of new Google \hyperlink{class_mock}{Mock} features.

\section*{Macros for Defining Actions}

\subsection*{Problem}

Due to the lack of closures in C++, it currently requires some non-\/trivial effort to define a custom action in Google \hyperlink{class_mock}{Mock}. For example, suppose you want to \char`\"{}increment the value pointed to by the
...
\begin{DoxyItemize}
\item They are non-\/standard and not widely installed. Google \hyperlink{class_mock}{Mock} only depends on standard libraries and {\ttfamily tr1\+::tuple}, which is part of the new C++ standard and comes with gcc 4+. We want to keep it that way.
