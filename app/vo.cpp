...
vo::vo() {
	...
	cv::Mat cameraMat = ((cv::Mat1d(3,3) << 964.828979, 0, 643.788025, 0, 964.828979, 484.407990, 0, 0, 1));
	...
	remap(Mat1, Mat1, lutxy[0], lutxy[1], cv::INTER_LINEAR, cv::BORDER_CONSTANT);
	...
	for (int j = i+1; j < i + 100; ++j) {
		...
		//Undistort image
		remap(Mat2, Mat2, lutxy[0], lutxy[1], cv::INTER_LINEAR, cv::BORDER_CONSTANT);

		detector->detectAndCompute( Mat2, cv::Mat(), keypoints_2, descriptors_2 );
		...
		E = findEssentialMat(scene, obj, focal, pp, cv::RANSAC, 0.999, 1.0, mask);
		...
		t_p = t_p + (R_p*t);
		R_p = R*R_p;
		std::cout << "Translational parameters are t[0] = " << t.at<double>(0) << ", t[1] = " << t.at<double>(1) << " and t[2] = " << t.at<double>(2) << std::endl;
...
void vo::computeGoodMatches(std::vector< cv::DMatch >& good_matches, cv::Mat& descriptors_1, cv::Mat& descriptors_2) {
	// using namespace cv;
